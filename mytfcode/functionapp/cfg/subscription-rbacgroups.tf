...
resource "azuread_group" "website-contributor" {
  display_name            = "AZ RBAC sub ${var.subscriptionname} Website Contributor"
  description             = "Manage websites, but not web plans.."
  prevent_duplicate_names = true
  security_enabled        = true
}
...
resource "azurerm_role_assignment" "subscriptionwebsite-contributor" {
  scope                = "/subscriptions/${var.subscriptionid}"
  role_definition_name = "Website Contributor"
  principal_id         = azuread_group.website-contributor.id
}
